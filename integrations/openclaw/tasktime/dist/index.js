...
const MCP_TOOL_DEFINITIONS = [
  ...
  },
  {
    ...
    inputSchema: {
      ...
      properties: {
        ...
        confirmCancel: optionalBoolean,
        confirmationText: optionalString,
        canceledAt: optionalNumber,
        idempotencyKey: optionalString
      },
  ...
  },
  {
    ...
    inputSchema: {
      type: "object",
      properties: { attemptId: optionalString },
      required: ["attemptId"],
      additionalProperties: false
    }
  },
...
const OPENCLAW_TOOL_DEFINITIONS = [...SETUP_TOOLS, ...MCP_TOOL_DEFINITIONS].map((tool) => ({
  ...tool,
  openClawName: `tasktime__${tool.name}`
})).sort((left, right) => left.openClawName.localeCompare(right.openClawName));
const OPENCLAW_TOOL_CONTRACT_NAMES = OPENCLAW_TOOL_DEFINITIONS.map((tool) => tool.openClawName);
class TaskTimeBridgeSupervisor {
  constructor(options = {}) {
  ...
  async start(context) {
    ...
    await mkdir(path.dirname(this.statusFile), { recursive: true, mode: 448 });
    try {
...
`, (error) => {
  ...
  handleBridgeLine(line) {
    ...
    if (response.error) {
      pending.reject(new Error(typeof response.error.message === "string" ? response.error.message : "bridge JSON-RPC request failed"));
      return;
    }
...
function createTaskTimeOpenClawPlugin() {
  return {
    id: PLUGIN_ID,
    name: "TaskTime Pro",
...
function detectLegacyTaskTimeMcpConflict(config) {
  ...
  return commandText.includes("tasktime-agent-bridge") || commandText.includes("@tasktimepro/agent-bridge") || commandText.includes("run-tasktime-agent-bridge");
}
function normalizePluginConfig(value) {
  ...
  return {
    ...
    commandTimeoutMs: readBoundedInteger(config.commandTimeoutMs, 12e4, 1e3, 3e5),
...
function normalizeMcpToolResult(result) {
  ...
  return {
    content: Array.isArray(result.content) ? result.content : [{ type: "text", text: JSON.stringify(result.structuredContent ?? result) }],
...
function sanitizeErrorMessage(error) {
  ...
  return message.replaceAll(/agentBridgePairing(?:Id|Code)=[^&\s]+/gi, "agentBridgePairingCredential=[redacted]").replaceAll(/\b\d{6}\b/g, "[redacted]").slice(0, 300);
}
...
const index = createTaskTimeOpenClawPlugin();
export {
  OPENCLAW_TOOL_CONTRACT_NAMES,
