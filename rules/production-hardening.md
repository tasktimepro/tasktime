...
- Treat browser input, imported backups, cloud-provider data, Worker responses, MCP requests, environment variables, files, and network messages as untrusted.
...
- Treat external request count as a production resource. For sync changes, compare connect, unchanged/no-op, local-edit, explicit-sync, reconnect, and recovery paths; preserve the lowest safe Worker/provider request count and add request-count assertions when behavior changes.
