...
- A temporary Google session-status failure must retain the session without treating it as a verified file transport. Only a supported direct policy permits provider connection; bounded status recovery respects Retry-After and does not send file requests to the retired Worker proxy. Authorization rejection still clears the invalid session.
- Google Drive and Dropbox recovery must remain bounded, coalesce consumers/wake signals, honor Retry-After, and time out status/token requests. Healthy wake checks add no auth traffic. Only temporary connection failures retry automatically; invalid auth, permissions, policy, malformed responses and conflicts stay explicit. Stale responses cannot reattach a disconnected/replaced session, and recovery cannot reset data or change the selected provider or sync mode.
...
- Connection, full-sync and lazy-document writers must serialize against each other, including concurrent loads of the same archive and environments without Web Locks. Callback-owned lazy loads serialize under the owning sync lock and drain before the outer pass resumes; a caught callback load failure still prevents successful sync status. Lazy loads subscribe to local edits before waiting or doing I/O so failure cannot detach dirty-data tracking.
