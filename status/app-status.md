...
- [ ] Complete the provider-neutral cloud-sync and Dropbox production rollout recorded in `status/cloud-sync-provider-expansion.md`. The compatible Worker and app are deployed, the remote hosted-identity migration and production configuration are applied, and Dropbox endpoints plus new connections are enabled while transfers remain fail-closed. The first production Edge canary reached a valid Google Drive move marker created by the local transfer test and exposed a recovery dead end. The locally validated compatibility fix stops retrying that source, makes connecting its recorded destination the primary action, and permits explicit source reuse only after verified deletion of all TaskTime source backups/sync files followed by one complete local push-only seed. Promotion and supported-browser production canaries remain open.
