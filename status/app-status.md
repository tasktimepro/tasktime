...
- [ ] Deliver the approved one-phase provider-neutral cloud-sync and Dropbox plan in the dependency order recorded in `status/cloud-sync-provider-expansion.md`; implementation through Slice 9 and provider-identity parity reconciliation is complete as the local core app `1.5.0` candidate, including provider-bound Worker control endpoints, memory-only browser tokens, direct App Folder file operations, one-active-provider lifecycle, Dropbox mode/backup parity, verified resumable transfer, provider-aware UI/agents, the shared Disconnect / Wipe data & disconnect lifecycle across Cloud Sync, Account, and agent deletion, active-provider hosted email/metrics, and an opaque identity link for future Pro continuity. The next-release client now enables Dropbox entry points by default, with an explicit false emergency opt-out, and next-release public/legal copy is reconciled in source. The owner-only development App Console/local D1 setup and complete local Microsoft Edge real-credential canary are green, including Manual/Backup/Sync behavior, direct-data-path request counts, backup lifecycle, two-tab convergence, offline recovery, verified wipe/revoke cleanup, and final Google reconciliation. Production supported-browser canaries, remote D1 migration, production secrets and compatible Worker deployment, and coordinated Worker-first enablement remain gated. Deployed Google Drive remains the current production contract.
...
- [x] Make Dropbox a default-on next-release client capability, retain only an explicit false emergency UI opt-out, and make onboarding provider-neutral while preserving truthful Drive-only fallback copy for that opt-out.
