...
- Provider-neutral cloud sync is now deployed with direct Google Drive and Dropbox data planes. The remote hosted-identity migration and production configuration are applied; Dropbox endpoints and new connections are enabled, while transfers remain disabled. A production Edge canary correctly reached a Google Drive move marker left by the local transfer test but exposed a recovery UX dead end. The locally validated fix treats the marker as an expected terminal fence, primarily offers the recorded Dropbox destination, and permits Google Drive reuse only through verified source-only deletion and a complete local push-only seed. The implementation is provider-symmetric and does not touch the recorded destination; promotion remains pending.
- The current moved-source release candidate is locally green: app lint, typecheck, merged production app/blog build, 239 test files / 2,232 tests with coverage (93.12% statements, 82.67% branches, 94.27% functions, and 94.19% lines), 39/39 Chromium smoke tests, 2/2 PWA smoke tests, private Worker typecheck and 13 files / 105 tests, plus bridge/OpenClaw package dry-runs and packaged bridge, managed-bundle, and live MCP smokes. The public Dropbox sync announcement is included in that verified build. This evidence does not close the production deployment or moved-source canary gate.
