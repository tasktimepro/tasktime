...
- Provider-neutral cloud sync with Dropbox is approved as one future sliced phase. The detailed plan preserves current Google behavior, permits one active provider per browser profile, adds verified provider transfer, keeps routine file bodies browser-to-provider, and hardens Cloudflare capacity before rollout; no Dropbox runtime behavior is implemented or advertised yet.
