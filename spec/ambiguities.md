...
- **DROPBOX-DEC-4 — Dropbox access (updated September 2, 2026):** Use Dropbox App Folder with `files.metadata.read`, `files.metadata.write`, `files.content.read`, and `files.content.write`, plus `account_info.read` solely so the browser can display the connected account email consistently with Google Drive. The browser reads that profile directly and stores the verified email only in its origin-local auth record; the Worker retains only the TaskTime-scoped pseudonym and exact scopes. Do not request Full Dropbox, sharing/team/contact, permanent-delete, or broader account access.
