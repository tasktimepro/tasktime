...
- **BILL-DEC-5 — OAuth consent:** Decide whether identity-only billing OAuth ships at launch or the existing Drive-scoped session is temporarily reused with clear disclosure.
...
- **BILL-DEC-9 — Support, retention, and repair:** Approve founder/beta/goodwill grants, trial-extension exceptions, audit/expiry rules, the minimal retained trial-eligibility marker after billing-profile deletion, and authenticated Google/Stripe account-link repair.
...
- **DROPBOX-DEC-3 — Cross-device boundary:** One active provider is strictly enforceable in the current browser profile and its same-origin tabs. Without a central TaskTime account, an offline device or legacy PWA cannot be switched atomically. Upgraded clients honor a non-sensitive migration marker, transfer warns users to close/update other devices, and provider revocation remains the dependable way to stop a legacy client from using the former grant. Public/UI wording may not claim a global atomic switch.
- **DROPBOX-DEC-4 — Dropbox access:** Use Dropbox App Folder with only `files.metadata.read`, `files.content.read`, and `files.content.write`. Do not request Full Dropbox, sharing/team/contact, permanent-delete, or account-info access unless a later approved need requires it.
...
- **DROPBOX-DEC-7 — Identity boundary:** Dropbox storage authentication never becomes billing, hosted-email, metrics, or product identity. Existing Google identity/session consumers stay compatible and remain governed by `BILL-DEC-5`.
...
- **DROPBOX-DEC-9 — Product boundary and rollout:** Supported cloud sync/backups remain in the approved free product boundary. Worker support deploys disabled first, followed by hidden client support, credential canary, bounded new connections, and only then transfers. After a user has Dropbox data, rollback may stop acquisition/transfer but must retain existing Dropbox access and may never silently switch them to Google.
