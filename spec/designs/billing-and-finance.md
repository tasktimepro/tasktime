...
- Increasing invoice hours above the canonical composer value creates a visible invoice-adjustment entry. Reducing hours below that value is rejected until the user splits or edits the source entries, so no unexplained whole-minute remainder is silently consumed. Validation reports every affected task by name in one notice and never exposes internal identifiers.
...
- Stored time intervals and timer elapsed values remain millisecond-exact. Timer and tracking displays may show seconds, but invoice-facing summaries and notices stop at whole minutes; display formatting never changes the stored interval.
- Billing snapshots retain the exact selected interval unless a configured project billing increment creates an explicit rounded-up `billableDurationMs` snapshot. Invoice presentation and pricing ignore any remaining source seconds before converting whole minutes to the existing two-decimal-hours value. That untouched canonical value is accepted without creating an adjustment; raw tracked time is not rewritten.
