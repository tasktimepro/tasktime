...
[ ] Coordinated license, app-origin migration, homepage, and production-launch program
    ...
    [ ] Program Phase 4 — Launch through controlled production changes
        ...
        [ ] Create/approve the separate site remote and initial history/provenance, license, maintainers, main-branch protection, required CI, and dependency monitoring; make site source durable before core main removes blog, and do not add either nested checkout to core Git
        ...
        [ ] Configure protected app/site deployment environments with distinct project targets, environment-scoped credentials, reviewed contract checksum, launch-readiness and split-deployment switches; first rehearse artifact-only runs and prove app-only/site-only changes cannot deploy the other component
