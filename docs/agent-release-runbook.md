...
- Core bridge discovery source: `agent-bridge/discovery/tasktime-agent.json` (site publishes its reviewed snapshot)
...
Do not commit the private key, PEM contents, or derived private-key hex. If the key is rotated, update `tasktime-site/public/.well-known/mcp-registry-auth` in the site repository, obtain the separate site deployment approval and verify the new public proof is live, then update `MCP_REGISTRY_PRIVATE_KEY` through its separately approved secret workflow.
...
Agent docs are generated by the independent `tasktime-site` repository under
`src/pages/agents/`. Core retains runtime/packages and the discovery source at
`agent-bridge/discovery/tasktime-agent.json`. Site serves the canonical manifest
and aliases from its reviewed `vendor/core-contract.json`, and owns the domain
verification proof. See `contracts/site-distribution.md` for producer/consumer
ownership and update compatibility.

For changed public tool/package metadata, export with `make site-contract`,
review the artifact diff, and explicitly update the site snapshot from the
approved committed source. Validate it against that exact core checkout with
`make npm CMD="run check:site-contract -- tasktime-site/vendor/core-contract.json"`.
Do not update or publish every agent artifact for a site-only edit.

Build and serve public docs independently (requires the optional site checkout):
...
# The workflow omits the token-only owner override and binds source-ref to the
# immutable GitHub Actions SHA for this trusted path.
gh workflow run publish-clawhub-plugin.yml --ref main \
