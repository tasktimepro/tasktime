...
| `PRIVACY.md:40` | Server audit data uses hashed identifiers/timestamps and does not archive content. | Preserve after cutover | Publish only with the EMAIL_DB minimization migration and log-removal evidence; do not overstate the current deployment before that cutover. |
