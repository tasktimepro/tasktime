...
export default function DashboardSummaryCards({ currentMonth, todayTime, todayLiveTime = 0, recentDays, dueCount, overdueCount, preferredCurrency, loading, error, navigateToInvoices }) {
    ...
    return (
        <section aria-label="Dashboard summary" tabIndex={0} className="-mx-4 flex min-w-0 snap-x snap-mandatory scroll-px-4 gap-3 overflow-x-auto px-4 pb-[2px] focus-visible:outline focus-visible:outline-2 focus-visible:outline-ring md:mx-0 md:grid md:grid-cols-2 md:overflow-visible md:px-0 md:pb-0 md:scroll-px-0 xl:grid-cols-4">
            {cards.map(card => {
