    ...
    return <ul className="divide-y divide-border">{activity.map(item => {
        ...
        return <li key={item.expense.id}>
            <button type="button" onClick={() => onView?.(item.expense)} disabled={!onView}
                ...
                <span className="min-w-0 flex-1"><span className="block text-xs font-medium">{item.label}</span><span className="mt-1 block truncate text-xs text-muted-foreground transition-colors group-hover:text-[hsl(var(--status-info-accent))] group-focus-visible:text-[hsl(var(--status-info-accent))]">{item.expense.title}</span></span>
                ...
                <div className="grid h-full min-w-0 grid-cols-1 gap-5 md:grid-cols-5">
                    ...
                    <div className="flex min-w-0 flex-col border-t pt-4 md:col-span-2 md:border-l md:border-t-0 md:pl-5 md:pt-0">
                        ...
                        {overview.breakdown.length ? <ul className="mt-4 space-y-4">{(allCategories ? overview.breakdown : overview.breakdown.slice(0, 5)).map(item => <li key={item.id} className="space-y-1.5">
                            <div className="flex items-start justify-between gap-3 text-xs"><span className="min-w-0 break-words">{item.name}</span><span className="shrink-0 text-right"><ExpenseAmount money={item.money} currency={currency} /></span></div>
                            {item.percentage !== null && <div className="flex items-center gap-2"><div className="h-1 flex-1 overflow-hidden rounded-full bg-muted"><div className="sensitive-data h-full rounded-full bg-[hsl(var(--status-info-accent))]" style={{ width: item.percentage + '%' }} /></div><span className="sensitive-data w-9 text-right text-xs text-muted-foreground">{Math.round(item.percentage)}%</span></div>}
