/**
 * YjsCloudSyncProvider - shared Google Drive and Dropbox sync engine
 *
...
const CLOUD_INCIDENT_THROTTLE_MS = 15 * 60 * 1000;
const CLOUD_VALIDATION_INCIDENT_THROTTLE_MS = 30 * 60 * 1000;
...
export class YjsCloudSyncProvider {
    ...
    private remoteValidationPending: boolean = false;
    private lastRemoteValidationWarning: string | null = null;
    ...
    private initializeOfflineConnection(reason: string): void {
        ...
        // document into an upload candidate. Backup mode may already have remote
    ...
    private captureIncident({
        ...
        error,
        context,
        throttleMs = CLOUD_INCIDENT_THROTTLE_MS,
    }: {
        incidentKey: string;
        message: string;
        error?: unknown;
    ...
    /**
     * Reject corrupt binary updates before touching the live document. Semantic
     * validation waits for the complete base/delta and cross-document pass.
     */
    applyValidatedRemoteUpdate(docName: DocName, doc: Y.Doc, update: Uint8Array, source: string): boolean {
        ...
        // Keep CRDT convergence even when references are temporarily incomplete.
        applyUpdate(doc, update, 'remote');
        this.remoteValidationPending = true;
        return true;
    }

    /** Whether the current manifest's complete remote revision is present locally. */
    private isRemoteDocumentReady(docName: DocName): boolean {
        const remote = this.manifest.getManifest()?.documents[docName];
        if (!remote) return true;
        if (!this.docManager.getDocSync(docName)) return false;
        if ((this.appliedStateVersions.get(docName) ?? 0) < remote.stateVersion) return false;
        const applied = this.appliedDeltaIds.get(docName);
        return remote.deltas.every(delta => applied?.has(delta.id));
    }

    /** Read-only safety check; never changes manual/backup pull behavior. */
    isWorkspaceHistoryReady(): boolean {
        const manifest = this.manifest.getManifest();
        return Boolean(manifest) && Object.keys(manifest!.documents)
            .every(name => this.isRemoteDocumentReady(name as DocName));
    }

    /** Diagnose settled data without mistaking unloaded dependencies for deleted records. */
    private validateSettledRemoteState(): void {
        if (!this.remoteValidationPending && !this.lastRemoteValidationWarning) return;
        const coreDoc = this.docManager.getDocSync('core');
        if (!coreDoc || this.docManager.getLoadedDocs().some(name => !this.isRemoteDocumentReady(name))) return;

        // Cloud absence does not imply local absence: disconnected history
        // can still live in an unloaded IndexedDB archive.
        const taskReferencesReady = Boolean(this.docManager.getDocSync('tasks-archived'))
            && this.isRemoteDocumentReady('tasks-archived');
        const invoiceReferencesReady = Boolean(this.docManager.getDocSync('invoices-archived'))
            && this.isRemoteDocumentReady('invoices-archived');
        try {
            validateDocManagerState(this.docManager, 'core', coreDoc, {
                taskReferencesReady,
                invoiceReferencesReady,
            });
            this.lastRemoteValidationWarning = null;
        } catch (error) {
            const message = error instanceof Error ? error.message : String(error);
            if (message !== this.lastRemoteValidationWarning) {
                console.warn(`[${this.providerDetails.logPrefix}] Validation warning after completed sync (updates retained):`, error);
                this.captureIncident({
                    ...
                    error,
                    context: { source: 'completed sync', docs: this.docManager.getLoadedDocs() },
                    throttleMs: CLOUD_VALIDATION_INCIDENT_THROTTLE_MS,
                });
                this.lastRemoteValidationWarning = message;
            }
        }
        // A later lazy archive load must finish the checks skipped above.
        this.remoteValidationPending = !taskReferencesReady || !invoiceReferencesReady;
    }

    /** Include documents opened by navigation while the initial connection is running. */
    private async syncLoadedDocumentsOnConnect(shouldPull: boolean, remoteOnly: boolean = false): Promise<void> {
        const processed = new Set<DocName>();
        while (true) {
            const next = this.docManager.getLoadedDocs().find(name => !processed.has(name));
            if (!next) {
                if (!remoteOnly && this.manifest.isDirty()) {
                    await this.manifest.save();
                    // Navigation may have loaded another document while the
                    // final manifest write was in flight. Finish it before ready.
                    if (this.docManager.getLoadedDocs().some(name => !processed.has(name))) continue;
                }
                return;
            }
            processed.add(next);
            // Subscribe before awaiting I/O so edits to the first document are retained too.
            this.subscribeToDoc(next);
            if (remoteOnly) {
                if (this.manifest.getManifest()?.documents[next]) {
                    const doc = this.docManager.getDocSync(next);
                    if (doc) await this.pullDoc(next, doc);
                }
            } else {
                await this.syncDoc(next, shouldPull);
            }
        }
    }
     ...
     *   'sync' = pull + push; 'backup' = initial reconciliation then push-only;
     *   'manual' = connect-only, except the optional pristine bootstrap pull
     */
    async connect(
    ...
    private async connectInner(syncMode?: CloudSyncMode, options: ConnectOptions = {}): Promise<void> {
        ...
        try {
            ...
            if (mode === 'sync') {
                ...
                if (hasRemoteData) {
                    // The initial Backup connection reconciles remote data once.
                    this.log('connect: first-sync pull for backup mode');
                    ...
                    await this.syncLoadedDocumentsOnConnect(false);
                ...
                // provider access. Pulling and pushing stay explicit so manual
                ...
                if (shouldBootstrapPull) {
                    ...
                    await this.syncLoadedDocumentsOnConnect(true, true);
            ...
            this.validateSettledRemoteState();
            this.connected = true;
            ...
            // Late edits may target a document already uploaded by this pass.
            // Retry only real local work, after the connection lock is released.
            if (this.connected && this.syncState !== 'error') {
                this.schedulePendingSyncRetry({ allowPull: this.syncMode === 'sync' });
            }
        }
    }
    ...
    /**
     * Disconnect from the selected cloud provider
     */
    disconnect(): void {
        ...
        this.appliedStateVersions.clear();
        this.appliedDeltaIds.clear();
        this.remoteValidationPending = false;
        this.lastRemoteValidationWarning = null;
        this.setState('idle');
        this.setPhase('idle');
        this.updatePendingState();
    ...
    /**
     * Wipe all TaskTime Pro sync files from the selected provider namespace
    ...
    /**
     * Check if connected to the selected cloud provider
     */
    isConnected(): boolean {
    ...
    private async syncInner(force: boolean, options: SyncOptions): Promise<void> {
        ...
        try {
            ...
            // An offline first connection has not inspected the provider yet. Complete
            ...
            if (completingInitialRemoteReconciliation) {
                ...
                if (manifestChanged || force) {
                    // Reload manifest from the provider to get latest changes and recover
            ...
            // Sync each loaded document when the provider has something to reconcile or
    ...
    private async pushDeltas(docName: DocName, doc: Y.Doc): Promise<boolean> {
        ...
        try {
            ...
            // Our uploaded delta is already present locally; any concurrently
            // discovered remote delta still requires a later pull.
            const applied = this.appliedDeltaIds.get(docName) ?? new Set<string>();
            applied.add(deltaId);
            this.appliedDeltaIds.set(docName, applied);
    ...
    private async pushFullState(docName: DocName, doc: Y.Doc, clearDeltas: boolean = false): Promise<boolean> {
        ...
        try {
            ...
            if (clearDeltas) this.appliedDeltaIds.set(docName, new Set());
    ...
    async syncAndSubscribeDoc(docName: DocName, options: { allowPull?: boolean } = {}): Promise<void> {
        ...
        if (!this.isSyncing) this.validateSettledRemoteState();
        this.updatePendingState();
    }
